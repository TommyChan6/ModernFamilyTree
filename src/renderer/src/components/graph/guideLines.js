...
function overlay(ctx) { return ctx.renderer && ctx.renderer.overlay }
function redraw(ctx) { if (ctx.requestRedraw) ctx.requestRedraw() }
...
function runGuideTimer(ctx, fn) {
  ...
  const t = d3.timer((el) => {
    if (fn(el)) { t.stop(); const i = ctx.guideTimers.indexOf(t); if (i >= 0) ctx.guideTimers.splice(i, 1) }
  })
  ctx.guideTimers.push(t)
}
export function cancelGuideTimers(ctx) {
  if (ctx.guideTimers) { ctx.guideTimers.forEach(t => t.stop()); ctx.guideTimers = [] }
  if (ctx.currentYearTimer) { ctx.currentYearTimer.stop(); ctx.currentYearTimer = null }
}
...
export function drawCurrentYearLine(ctx, ageInfo, year, animate = true) {
  ...
  if (!ov.currentYear) {
    ...
    ctx.currentYearTimer = d3.timer((el) => {
      if (ov.currentYear !== line) { ctx.currentYearTimer?.stop(); ctx.currentYearTimer = null; return true } // superseded/removed
      line.opacity = Math.min(1, el / 400); redraw(ctx)
      if (line.opacity >= 1) { ctx.currentYearTimer = null; return true }
      return false
    })
    return
  }
  ...
  if (animate) {
    const startY = line.y
    ctx.currentYearTimer = d3.timer((el) => {
      if (ov.currentYear !== line) { ctx.currentYearTimer?.stop(); ctx.currentYearTimer = null; return true }
      const t = Math.min(1, d3.easeCubicInOut(el / 450))
      line.y = startY + (y - startY) * t; redraw(ctx)
      if (t >= 1) { ctx.currentYearTimer = null; return true }
      return false
    })
  } else {
    line.y = y
...
export function removeCurrentYearLine(ctx) {
  if (ctx.currentYearTimer) { ctx.currentYearTimer.stop(); ctx.currentYearTimer = null }
  const ov = overlay(ctx)
  if (ov) { ov.currentYear = null; redraw(ctx) }
}
...
export function drawYearGuides(ctx, minYear, maxYear, padding, usableHeight) {
  ...
  for (let year = startYear; year <= endYear; year += interval) {
    ...
    guides.push({ y: padding + yRatio * usableHeight, label: String(year), kind: 'year', opacity: 0 })
  }
  ov.guides = guides
...
export function removeGenPreview(ctx) {
  const ov = overlay(ctx)
  if (ov) { ov.genPreview = null; redraw(ctx) }
}
...
export function redistributeGenRows(ctx, tickedFn) {
  ...
  ctx.nodesData.forEach(n => {
    ...
    oldSorted.forEach(oy => { const d = Math.abs(n.y - oy); if (d < closestDist) { closestOld = oy; closestDist = d } })
  ...
  runGuideTimer(ctx, (elapsed) => {
    ...
    ctx.nodesData.forEach(n => {
      const sy = nodeStartY[n.id], ey = nodeTargetY[n.id]
      if (sy === undefined || ey === undefined) return
      n.y = sy + (ey - sy) * t; n.fy = n.y
    })
    ov.guides = oldSorted.map((oldY, i) => ({
      y: oldY + (newYValues[i] - oldY) * t, label: `Gen ${i + 1}`, kind: 'gen', opacity: 1,
    }))
    tickedFn()
    if (t >= 1) {
      ctx.nodesData.forEach(n => { if (nodeTargetY[n.id] !== undefined) { n.y = nodeTargetY[n.id]; n.fy = n.y } })
      ov.guides = newYValues.map((y, i) => ({ y, label: `Gen ${i + 1}`, kind: 'gen', opacity: 1 }))
      tickedFn()
      return true
    }
