...
export class LinkLayer {
  ...
  get arrowObject3d() { return this.arrowMesh }
  ...
  updateGeometry(links, gs, visual) {
    ...
    for (let j = 0; j < links.length; j++) {
      ...
      for (let i = 0; i <= SEG; i++) {
        const p = points[i]
        const prev = points[Math.max(0, i - 1)], next = points[Math.min(SEG, i + 1)]
      ...
      if (vis.arrowColorRGB) {
        const T = points[SEG]
        let dx = T.x - control.x, dy = T.y - control.y
        ...
        this.aColor[j * 3] = col[0]; this.aColor[j * 3 + 1] = col[1]; this.aColor[j * 3 + 2] = col[2]
      } else {
        this.aScale[j] = 0
      }
    }
  ...
  updateStyles(links, visual) {
    ...
    for (let j = 0; j < links.length; j++) {
      ...
      const dl = vis.dashLen || 0, gl = vis.dashGap || 0
      const base = j * VPL
      for (let v = 0; v < VPL; v++) {
        const vi = base + v
        this.color[vi * 3] = col[0]; this.color[vi * 3 + 1] = col[1]; this.color[vi * 3 + 2] = col[2]
        this.opacity[vi] = op
        this.dash[vi * 2] = dl; this.dash[vi * 2 + 1] = gl
      }
    }
  ...
  dispose() {
    this.geometry?.dispose(); this.arrowGeometry?.dispose()
    this.material?.dispose(); this.arrowMaterial?.dispose()
  }
}
