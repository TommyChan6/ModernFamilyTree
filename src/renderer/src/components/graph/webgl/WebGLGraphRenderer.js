...
export class WebGLGraphRenderer {
  constructor({ glCanvas, overlayCanvas, hooks }) {
    ...
    this.atlas = new AvatarAtlas(() => { this.markNodeStylesDirty(); this.requestRedraw() })
    ...
    this._onContextRestored = () => {
      ...
      this.markAllDirty(); this.requestRedraw()
    }
  ...
  markNodeStylesDirty() { this.nodeStylesDirty = true }
  markLinkStylesDirty() { this.linkStylesDirty = true }
  markAllDirty() { this.nodeStylesDirty = true; this.linkStylesDirty = true }

  setCamera(t) { this.transform = t; this.overlay.setCamera(t); this.requestRedraw() }
  ...
  invalidatePicker() { this.picker.invalidate() }
  pickNode(wx, wy, radius) { return this.picker.pickNode(wx, wy, this.nodes || [], radius) }
  pickLink(wx, wy, gs) { return this.picker.pickLink(wx, wy, this.links || [], gs) }
  ...
  // ── Node styling / tween ────────────────────────────────────────────────────
  _syncNodeStyles() {
    const nodes = this.nodes || [], visual = this.hooks.nodeVisual
    this._nodeCache.length = nodes.length
    this._nodeIds.length = nodes.length
    const seen = new Set()
    for (let i = 0; i < nodes.length; i++) {
      const n = nodes[i], v = visual(n)
      const layer = v.imageUrl ? this.atlas.request(n.id, v.imageUrl) : -1
      v.avatar = layer >= 1 ? layer : 0 // 0 = silhouette fallback
      v.fillRGB = rgb(v.fill); v.borderRGB = rgb(v.border)
      this._nodeCache[i] = v
      this._nodeIds[i] = n.id
      seen.add(n.id)
      let a = this._nodeAnim.get(n.id)
      if (!a) { a = { op: 0, rad: v.radius, glow: 0 }; this._nodeAnim.set(n.id, a) } // new -> fade in
      a.top = v.opacity; a.trad = v.radius; a.tglow = v.glow
    }
  ...
  _stepNodes(dt) {
    ...
    let moving = false
    for (let i = 0; i < nodes.length; i++) {
      const v = this._nodeCache[i], a = this._nodeAnim.get(this._nodeIds[i])
      if (!a) continue
      a.op = approach(a.op, a.top, dt); a.rad = approach(a.rad, a.trad, dt); a.glow = approach(a.glow, a.tglow, dt)
      if (Math.abs(a.op - a.top) > TWEEN_EPS || Math.abs(a.rad - a.trad) > TWEEN_EPS || Math.abs(a.glow - a.tglow) > TWEEN_EPS) moving = true
      this.nodeLayer.setStyle(i, {
        radius: a.rad, fill: v.fillRGB, border: v.borderRGB, borderPx: v.borderPx,
        borderA: v.borderA, opacity: a.op, selected: v.selected, glow: a.glow, avatar: v.avatar,
      })
    }
  ...
  // ── Link styling / tween ────────────────────────────────────────────────────
  _syncLinkStyles() {
    const links = this.links || [], visual = this.hooks.linkVisual
    ...
    for (const d of links) {
      ...
      if (!a) a = { op: 0, w: v.width, ar: v.arrowSize, cr: v.colorRGB[0], cg: v.colorRGB[1], cb: v.colorRGB[2] }
      a.top = v.opacity; a.tw = v.width; a.tar = v.arrowSize
      a.tcr = v.colorRGB[0]; a.tcg = v.colorRGB[1]; a.tcb = v.colorRGB[2]
  ...
  _stepLinks(dt) {
    ...
    for (const a of this._linkAnim.values()) {
      a.op = approach(a.op, a.top, dt); a.w = approach(a.w, a.tw, dt); a.ar = approach(a.ar, a.tar, dt)
      a.cr = approach(a.cr, a.tcr, dt); a.cg = approach(a.cg, a.tcg, dt); a.cb = approach(a.cb, a.tcb, dt)
      if (Math.abs(a.op - a.top) > TWEEN_EPS || Math.abs(a.w - a.tw) > TWEEN_EPS || Math.abs(a.ar - a.tar) > TWEEN_EPS ||
          Math.abs(a.cr - a.tcr) > TWEEN_EPS || Math.abs(a.cg - a.tcg) > TWEEN_EPS || Math.abs(a.cb - a.tcb) > TWEEN_EPS) moving = true
    }
    return moving
  }
  ...
  _curLinkVisual(d) {
    const a = this._linkAnim.get(d.id), v = this._linkCacheById.get(d.id)
    if (!a || !v) return { color: [0.5, 0.5, 0.5], opacity: 0, dashLen: 0, dashGap: 0, width: 1, arrowColorRGB: null, arrowSize: 0 }
    return {
      color: [a.cr, a.cg, a.cb], opacity: a.op, dashLen: v.dashLen, dashGap: v.dashGap,
      width: a.w, arrowColorRGB: v.arrowRGB, arrowSize: a.ar,
    }
  }
  ...
  _frame(ts) {
    ...
    if (this.nodeStylesDirty) { this._syncNodeStyles(); this.nodeStylesDirty = false; this._nodeTweening = true }
    if (this.linkStylesDirty) { this._syncLinkStyles(); this.linkStylesDirty = false; this._linkTweening = true; this._linkStyleWrite = true }
    ...
    if (this._linkTweening || this._linkStyleWrite) { this.linkLayer.updateStyles(links, this._curLinkVisual); this._linkStyleWrite = false }
  ...
  dispose() {
    ...
    this.atlas.dispose(); this.nodeLayer.dispose(); this.linkLayer.dispose()
