...
const { src: avatarSrc, loading: avatarLoading } = useThumbnail(() => props.person.primary_image || '')
...
.pcard-type {
  ...
  color: #fff; background: rgba(0, 0, 0, 0.28); padding: 3px 8px; border-radius: 20px;
}
...
.pcard:hover .pcard-avatar { transform: scale(1.06); }
.pcard-avatar-img {
  width: 100%; height: 100%; object-fit: cover;
  animation: pcard-avatar-in 0.35s ease both;
}
@keyframes pcard-avatar-in { from { opacity: 0; } }
...
@media (prefers-reduced-motion: reduce) {
  .pcard-avatar-img { animation: none; }
  .pcard-avatar.is-loading::after { animation: none; }
}
...
.pcard-stats {
  ...
  width: calc(100% - 24px);
  margin: auto 0 0; padding-top: 10px; border-top: 1px solid var(--border);
}
